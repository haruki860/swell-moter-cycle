        ...
        <ul className="imageList">
            {props.currentBooks &&
                props.currentBooks.map((book) => {
                    return<li key={book.image_url}><img src={book.image_url} className="bookListImage"/></li>
                })}
        </ul>
    );
};
...
const Gallery = () => {
    ...
    const getBookURLs = async (storageRef) => {
        ...
        return await Promise.all(itemNames.map(async (name) => await getBookURL(storageRef, name)));
    }
